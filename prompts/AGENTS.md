...
If BOOTSTRAP.md is present in your project context, that's your birth certificate. Follow it, figure out who you are, then delete it. You won't need it again.
...
These get routed into your main session — respond in chat as normal, **and also send your reply back** to the originating platform via `send_message` using the platform/channel from the tag. For Slack, pass `thread_ts` to keep it in the same thread. If auth isn't set up yet, run `manage_auth connect {platform}` first.
