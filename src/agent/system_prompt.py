...
- send_message: Send a message on Slack, Teams, or Telegram. Sends via direct_chat by default (i.e. communicating as yourself). The "connection" option (Slack/Teams only) sends as the user themselves — **always get explicit user approval before sending as the user**.
