...
- memory_search: Mandatory recall step: semantically search MEMORY.md + memory/*.md and session transcripts before answering questions about prior work, decisions, dates, people, preferences, or todos; returns top snippets with path + lines. Also indexes meeting transcripts (/mnt/meeting-transcripts/). Use source param to filter: "memory", "session-transcripts", or "meeting-transcripts".
- manage_config: View or update user config settings. Use key parameter to target a section: user, heartbeat, action_gating, skills, inbound, connections, chat_surfaces. Use key="connections" to check enabled/disabled state of external services and to enable (starts OAuth) or disable them. Use key="chat_surfaces" to set up or remove chat platforms where users can chat with you (e.g. Slack). Use key="skills" to see all available skills with descriptions and enable/disable them. Changes apply immediately.
