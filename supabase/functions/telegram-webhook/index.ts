 ...
 * On /start command, stores the user's chat ID in vault to enable the direct chat.
 *
 * Routes:
...
async function handleUpdate(update: Record<string, unknown>): Promise<Response> {
  ...
  // Handle /start command — register the direct chat
  ...
  if (ownerChatId && ownerChatId !== chatId) {
    await sendTelegramMessage(chatId, DIRECT_CHAT_DISABLED_REPLY);
    return new Response("ok");
  }
