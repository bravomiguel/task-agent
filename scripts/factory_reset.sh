...
from supabase import create_client
sb = create_client(os.environ['SUPABASE_URL'], os.environ['SUPABASE_SERVICE_KEY'])
for name in ['slack_bot_token', 'slack_bot_user_id', 'slack_signing_secret', 'slack_bot_owner_id', 'teams_bot_app_id', 'teams_bot_app_secret', 'teams_bot_tenant_id', 'teams_bot_home_tenant_id', 'teams_bot_service_url', 'teams_bot_owner_conversation_id', 'teams_webhook_secret', 'telegram_owner_chat_id', 'telegram_owner_user_id', 'telegram_owner_name', 'inbound_sources']:
    try:
